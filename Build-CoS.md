...
1. **Read the companion file `CLAUDE.md`** in this same folder. It establishes the foundational operating rules for this project — the **WATC architecture** (Workflows, Agents, Tools, Context), the self-improvement loop, and the principle that probabilistic AI handles reasoning while deterministic code handles execution. These rules apply to everything you do in this folder, including the setup work in this guide.
...
tools/                              # Empty — deterministic scripts as you wire integrations
...
Single source of truth for tasks. Organized by context.
...
Written at the end of every session. Captures what happened and what is open.
...
- What does your weekly rhythm look like? (e.g., Monday planning, Friday review, daily standup)
...
(from Section 2 + Section 9 Connections)
...
Pull from Section 3. Cover methodology, weekly rhythm, where work lives, reporting habit, deliverable standards. Reference any tools mentioned in Section 2 / Section 9 Connections.
...
Take the top items from Section 4 and combine with anything urgent from Section 9 Capabilities. Date it today. Format:
...
(The single most important thing right now. Pull from Section 1's #1 priority + any urgent item from Sections 4 or 9 Capabilities. One paragraph, max.)
...
(Pull from Section 4 and Section 9 Cadence. Bullet list of what's planned.)
...
Take items from Section 9 Capabilities and seed them under the right headings. Most go under "Backlog" until scheduled.
...
When any action would result in an outbound email being sent on the user's behalf:

1. Create the email as a **draft** in the user's email tool (or write the draft as a file in the project if no integration is wired yet).
2. Tell the user the draft is ready, with the recipients, subject, and a one-line summary.
3. Stop. Do not send, schedule, or otherwise dispatch the message.

The user reviews drafts and manually presses Send. No exceptions.

(If you later wire an email integration, encode this rule in the tool itself so it cannot send, only draft.)
```
...
{If they named any in Section 9 Cadence — morning brief at 7, weekly review Friday 4pm, drift flag Sundays — list them here. These will become routines or skills.}
...
Do not put communication style, CoS behavior, or email policy in `CLAUDE.md` directly — those live in `.claude/rules/`.
